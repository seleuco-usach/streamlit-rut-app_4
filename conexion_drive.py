...
spreadsheet0 =client.open_by_key('1pNYZhq9fvtRUG_XAPaZJqyDKSgzY6arl_tfSsMzq2vw')
worksheet = spreadsheet0.get_worksheet(0)  # índice 0 es la primera hoja
data0 = worksheet.get_all_values()

PAC_CARGA_2= pd.DataFrame(data0[1:], columns=data0[0])
...
set_with_dataframe(spreadsheet.add_worksheet(title="NuevaHoja", 
                                             rows="100", cols="20"), 
                   resultado)



len(PAC_CARGA_2)##3408
len(PAC_CARGA)##3384


PAC_CARGA_2[['NUM_DOCUMENTO',
             'PRIMER_APELLIDO',
             'SEGUNDO_APELLIDO',
             'VIGENCIA',
             'NUM_HORAS_PLANTA',
             'NUM_HORAS_CONTRATA',
             'NUM_HORAS_HONORARIOS']][~PAC_CARGA_2['NUM_DOCUMENTO'].isin(PAC_CARGA['rut'])]


type(PAC_CARGA_2['NUM_DOCUMENTO'])
type(PAC_CARGA['rut'])
